...
function [E1_x,E1_y,E1_z,H1_x,H1_y,H1_z,E2_x,E2_y,E2_z,H2_x,H2_y,H2_z] = parallel_polarizaton(theta,e,eps1,eps2,beta1,beta2)
...
e_hat_i = [cosd(theta_i) 0 -sind(theta_i)];
...
Gamma=(eta(2)*cosd(theta_t) - eta(1)*cosd(theta_i))/(eta(2)*cosd(theta_t) + eta(1)*cosd(theta_i));

T = (2*eta(2)*cosd(theta_i))/(eta(2)*cosd(theta_t) + eta(1)*cosd(theta_i));
...
e_hat_r = [cosd(theta_r) 0 sind(theta_r)];
h_hat_r = cross(n_hat_r,e_hat_r);
...
e_hat_t = [cosd(theta_t) 0 -sind(theta_t)];
h_hat_t = cross(n_hat_t,e_hat_t);
